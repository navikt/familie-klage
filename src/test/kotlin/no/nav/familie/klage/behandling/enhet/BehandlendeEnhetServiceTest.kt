...
class BehandlendeEnhetServiceTest {
    ...
    inner class OppdaterBehandlendeEnhet {
        ...
        fun `skal oppdatere behandlende enhet på behandling og oppgave samt opprette historikkinnslag`(nyBehandlendeEnhet: BarnetrygdEnhet){
            // Arrange
            ...
            val oppdaterBehandlendeEnhetRequest = OppdaterBehandlendeEnhetRequest(
            ...
            every { behandlingService.oppdaterBehandlendeEnhet(behandling.id, capture(behandlendeEnhetSlot)) } just Runs
            every {
            ...
            // Assert
            verify(exactly = 1) {behandlingService.oppdaterBehandlendeEnhet(any(), any())}
        ...
        fun `skal oppdatere behandlende enhet på behandling og oppgave samt opprette historikkinnslag`(nyBehandlendeEnhet: KontantstøtteEnhet){
            // Arrange
        ...
        fun `skal kaste feil dersom fagsystem er EF`(stønadstype: Stønadstype){
            ...
            val behandlendeEnhetSlot = slot<String>()
            val historikkHendelseSlot = slot<HistorikkHendelse>()
            val beskrivelseSlot = slot<String>()
            val behandlendeEnhetOppgaveSlot = slot<Enhet>()
            ...
            // Act & Assert
            val feil = assertThrows<ApiFeil> { behandlendeEnhetService.oppdaterBehandlendeEnhet(behandling.id, oppdaterBehandlendeEnhetRequest) }

            assertThat(feil.message).isEqualTo("Støtter ikke oppdatering av behandlende enhet for fagsystem EF")
